...
public class ComportamentoDetran extends SimpleBehaviour {
	...
	private void setServidores() throws MalformedURLException, IOException {
		...
		if (resp) {
			retornoFinal += this.getAgent().getLocalName() + ": Aba 'Servidores': Link e arquivos válidos.\n";
		} else {
			retornoFinal += this.getAgent().getLocalName() + ": Aba 'Servidores': Link e/ou arquivos inválidos.\n";
		}
	}
	...
	private void setAcoesProgramas() throws MalformedURLException, IOException {
		...
		if (resp) {
			...
			retornoFinal += this.getAgent().getLocalName() + ": Aba 'Acoes e Programas' não está correta.\n";
		}
	}
	...
	private void setExecucaoOrcamentariaExtensao() throws MalformedURLException, IOException {
		...
		if (resp) {
			retornoFinal += this.getAgent().getLocalName() + ": Aba 'Execução Orçamentária: Extensões corretas.\n";
		} else {
			retornoFinal += this.getAgent().getLocalName() + ": Aba 'Execução Orçamentária: Extensões incorretas.\n";
		}
	}
	...
	public boolean done() {
		if (isFinished) {
			System.out.println("Fim execução agente Detran");
			myAgent.doDelete();
		} else {
		}
		return false;
